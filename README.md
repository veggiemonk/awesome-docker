...
- [Dockerfile Sanity](https://github.com/sujeito-operator/dockerfile-sanity) - Dockerfile linter with no dependencies and nothing to install first: runs as a GitHub Action, a CLI or a VS Code extension. Checks build-cache ordering, credentials baked into ENV or ARG, and bytes a later `rm` cannot unship from an earlier layer.
